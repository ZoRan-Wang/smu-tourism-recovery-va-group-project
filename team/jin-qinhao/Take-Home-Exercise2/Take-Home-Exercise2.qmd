...
processed_dir <- if (artifact_export_enabled) {
  data_dir
} else {
  file.path(tempdir(), "take_home_ex2_artifacts", "data")
}
...
```{r}
output_dir <- if (artifact_export_enabled) {
  file.path(module_dir, "outputs")
} else {
  file.path(tempdir(), "take_home_ex2_artifacts", "outputs")
}
