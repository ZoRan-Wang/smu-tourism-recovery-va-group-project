...
artifact_export_enabled <- identical(
  tolower(Sys.getenv("WRITE_DECISION_TREE_ARTIFACTS", "false")),
  "true"
)
artifact_root <- if (artifact_export_enabled) {
  file.path(project_root, "artifacts")
} else {
  file.path(tempdir(), "decision_tree_artifacts")
}
processed_dir <- file.path(artifact_root, "tables")
...
```{r}
data_path <- file.path(processed_dir, "tourism_decision_tree_ready.csv")
output_dir <- file.path(artifact_root, "plots")
...
The prototype page keeps the full `fit_tree` in the interactive view so the deeper split path remains visible. The smaller tree shown previously came from `pruned_tree`, which removes the later `arrivals_million` split after cross-validation pruning.
